...
class Enrich(beam.DoFn):
    ...
    def process(self, element, side_input):
        result = dict(element)
        result["user_name"] = side_input[result["user_id"]]
        yield result
...
class EnrichOrderFn(beam.DoFn):
    ...
    def __init__(self, uri):
        ...
        self.tag = 0
        self.max_stale_sec = 60
        self.handle = shared.Shared()

    def get_tag(self):
        current_ts = self.datetime.now().timestamp()
        return current_ts - (current_ts % self.max_stale_sec)
    ...
    def load_users(self):
        ...
        with blob.open() as f:
            for line in f.readlines():
                ...
                result[kv[0]] = kv[1]
        logging.info("Lookup data updated")
        return self.WeakRefDict(result)

    def start_bundle(self):
        new_tag = self.get_tag()
        if new_tag > self.tag:
            self.users_lookup = self.handle.acquire(self.load_users, new_tag)
            self.tag = new_tag
